		...
		try {
			...
			for(OWLAxiom ax : ontology.getAxioms()) {
				...
				if (ax.getAxiomType() == AxiomType.SUBCLASS_OF) {
					uri = ((OWLSubClassOfAxiom) ax).getSubClass().asOWLClass().getURI().toString();
					if (uri.equals("http://www.langenmaier.de/u2r3/sample.owl#Book")) {
			...
			manager.saveOntology(ontology, new DefaultOntologyFormat(), physicalURI2);
