...
public class PrpRngLitRule extends ApplicationRule {
	static Logger logger = Logger.getLogger(PrpRngLitRule.class);
	
	PrpRngLitRule(U2R3Reasoner reasoner) {
		super(reasoner);
	...
	protected String buildQuery(DeltaRelation delta, DeltaRelation newDelta,
		...
		if (settings.getDeletionType() == DeletionType.CASCADING) {
			...
			sql.append("\n\t SELECT ass.object, rng.range, MIN(ass.id) AS sourceId1, '" + RelationName.dataPropertyAssertion + "' AS sourceTable1, MIN(rng.id) AS sourceId2, '" + RelationName.propertyDomain + "' AS sourceTable2");
		} else {
		...
		if (again) {
			...
			sql.append("\n\t\t WHERE bottom.entity = ass.object AND bottom.class = rng.range");
			sql.append("\n\t )");
		}
	...
	public String toString() {
		return "classAssertionEnt(Y, C) :- propertyRange(P, C), dataPropertyAssertion(X, P, Y)";
	}
