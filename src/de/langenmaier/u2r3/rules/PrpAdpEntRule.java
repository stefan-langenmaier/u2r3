...
public class PrpAdpEntRule extends ConsistencyRule {
	static Logger logger = Logger.getLogger(PrpAdpEntRule.class);
	
	PrpAdpEntRule(U2R3Reasoner reasoner) {
		super(reasoner);
		targetRelation = null;
	...
	protected String buildQuery(DeltaRelation delta, DeltaRelation newDelta,
		...
		if (run == 0) {
			sql.append("\n\t INNER JOIN " + delta.getDeltaName("objectPropertyAssertion") + " AS prp1 ON l.element = prp1.property");
			sql.append("\n\t INNER JOIN objectPropertyAssertion AS prp2 ON l.element = prp2.property");
		} else if (run == 1) {
			sql.append("\n\t INNER JOIN objectPropertyAssertion AS prp1 ON l.element = prp1.property");
			sql.append("\n\t INNER JOIN " + delta.getDeltaName("objectPropertyAssertion") + " AS prp2 ON l.element = prp2.property");
		}
