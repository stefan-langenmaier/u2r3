...
public class EqRepSEntRule extends ApplicationRule {
	...
	protected String buildQuery(DeltaRelation delta, DeltaRelation newDelta,
		...
		if (settings.getDeletionType() == DeletionType.CASCADING) {
			...
			sql.append("\n\t SELECT sa.right, ass.property, ass.object, MIN(sa.id) AS sourceId1, '" + RelationName.sameAsEnt + "' AS sourceTable1, MIN(ass.id) AS sourceId2, '" + RelationName.objectPropertyAssertion + "' AS sourceTable2");
		} else {
		...
		sql.append("\n\t\t INNER JOIN " + delta.getDeltaName("objectPropertyAssertion") + " AS ass ON sa.left = ass.subject");
