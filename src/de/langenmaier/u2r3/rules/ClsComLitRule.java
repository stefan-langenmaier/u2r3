...
public class ClsComLitRule extends ConsistencyRule {
	...
	protected String buildQuery(DeltaRelation delta, DeltaRelation newDelta,
		...
		if (run == 0) {
			sql.append("\n\t INNER JOIN " + delta.getDeltaName("classAssertionLit") + " AS ca1 ON co.colLeft = ca1.colClass");
			sql.append("\n\t INNER JOIN classAssertionLit AS ca2 ON co.colRight = ca2.colClass");
		} else if (run == 1) {
			sql.append("\n\t INNER JOIN classAssertionLit AS ca1 ON co.colLeft = ca1.colClass");
			sql.append("\n\t INNER JOIN " + delta.getDeltaName("classAssertionLit") + " AS ca2 ON co.colRight = ca2.colClass");
		}
