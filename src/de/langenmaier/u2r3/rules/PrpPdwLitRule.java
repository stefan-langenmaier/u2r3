...
public class PrpPdwLitRule extends ConsistencyRule {
	...
	protected String buildQuery(DeltaRelation delta, DeltaRelation newDelta,
		...
		if (run == 0) {
			...
			sql.append("\n\t INNER JOIN dataPropertyAssertion AS prp2 ON pdw.colRight = prp2.property");
		} else if (run == 1) {
			sql.append("\n\t INNER JOIN dataPropertyAssertion AS prp1 ON pdw.colLeft = prp1.property");
			sql.append("\n\t INNER JOIN " + delta.getDeltaName("dataPropertyAssertion") + " AS prp2 ON pdw.colRight = prp2.property");
		}
