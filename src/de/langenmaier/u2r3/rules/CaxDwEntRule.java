...
public class CaxDwEntRule extends ConsistencyRule {
	...
	protected String buildQuery(DeltaRelation delta, DeltaRelation newDelta,
		...
		if (run == 0) {
			sql.append("\n\t INNER JOIN " + delta.getDeltaName("classAssertionEnt") + " AS ca1 ON dw.colLeft = ca1.class");
			sql.append("\n\t INNER JOIN classAssertionEnt AS ca2 ON dw.colRight = ca2.class");
		} else if (run == 1) {
			sql.append("\n\t INNER JOIN classAssertionEnt AS ca1 ON dw.colLeft = ca1.class");
			sql.append("\n\t INNER JOIN " + delta.getDeltaName("classAssertionEnt") + " AS ca2 ON dw.colRight = ca2.class");
		}
