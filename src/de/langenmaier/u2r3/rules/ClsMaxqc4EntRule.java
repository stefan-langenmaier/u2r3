...
public class ClsMaxqc4EntRule extends ApplicationRule {
	static Logger logger = Logger.getLogger(ClsMaxqc4EntRule.class);
	
	ClsMaxqc4EntRule(U2R3Reasoner reasoner) {
		super(reasoner);
	...
	protected String buildQuery(DeltaRelation delta, DeltaRelation newDelta,
		...
		if (settings.getDeletionType() == DeletionType.CASCADING) {
			...
			sql.append(" MIN(prp1.id) AS sourceId5, '" + RelationName.objectPropertyAssertion + "' AS sourceTable5, ");
			sql.append(" MIN(prp2.id) AS sourceId6, '" + RelationName.objectPropertyAssertion + "' AS sourceTable6");
		} else {
		...
		if (run == 0) {
			...
			sql.append("\n\t\t INNER JOIN " + delta.getDeltaName("objectPropertyAssertion") + " AS prp2 ON ca1.entity = prp2.subject AND op.property = prp2.property");
		}
	...
	public String toString() {
		return "sameAsEnt(Y1, Y2) :- maxQualifiedCardinality(X, 1), onProperty(X, P), onClass(X, thing), classAssertionEnt(U, X), objectPropertyAssertion(U, P, Y1), objectPropertyAssertion(U, P, Y2)";
	}
