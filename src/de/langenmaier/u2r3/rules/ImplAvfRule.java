...
public class ImplAvfRule extends ApplicationRule {
	static Logger logger = Logger.getLogger(ImplSvfRule.class);
	
	ImplAvfRule(U2R3Reasoner reasoner) {
		super(reasoner);
	...
	protected String buildQuery(DeltaRelation delta, DeltaRelation newDelta,
		...
		if (again) {
			...
			sql.append("\n\t\t WHERE bottom.entity = avf.part AND bottom.colClass = '" +  clazz + "'");
			sql.append("\n\t )");
		}
	...
	public String toString() {
		return "classAssertionEnt(X, Restriction) :- allValuesFrom(X, P, C)";
	}
