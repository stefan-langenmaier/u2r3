...
public class ScmDpSubRule extends ApplicationRule {
	static Logger logger = Logger.getLogger(ScmDpSubRule.class);
	
	ScmDpSubRule(U2R3Reasoner reasoner) {
		super(reasoner);
	...
	protected String buildQuery(DeltaRelation delta, DeltaRelation newDelta, boolean again, int run) {
		...
		sql.append("\n\t WHERE ca.colClass = '" + OWLRDFVocabulary.OWL_DATA_PROPERTY + "'");
